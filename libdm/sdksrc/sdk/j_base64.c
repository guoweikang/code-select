...
{
	unsigned char *data=(unsigned char *)plat_malloc(size);
	memset(data, 0, size);
	return data;
	return (unsigned char *)plat_malloc(size);
  ...
  *encData = output = base64_malloc(output_size);
  ...
  while(dataLen > 0)
  {
    ...
    case 1: /* only one byte read */
      plat_snprintf((char*)output, 5, "%c%c==",
               table64[obuf[0]],
               table64[obuf[1]]);
      break;
    case 2: /* two bytes read */
      plat_snprintf((char*)output, 5, "%c%c%c=",
               table64[obuf[0]],
               table64[obuf[1]],
               table64[obuf[2]]);
      break;
    default:
      plat_snprintf((char*)output, 5, "%c%c%c%c",
               table64[obuf[0]],
               table64[obuf[1]],
               table64[obuf[2]],
               table64[obuf[3]]);
