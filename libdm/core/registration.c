                                        ...
                                        void * message)
{
    ...
    if (targetP->status == STATE_REG_PENDING)
    {
        ...
        else
        {
            ...
            lwm2m_sdk_notify_reg_failed(packet?packet->code:COAP_IGNORE,"Registration failed");
        }
    }
}
                                              ...
                                              void * message)
{
    ...
    if (targetP->status == STATE_REG_UPDATE_PENDING)
    {
        ...
        else
        {
            ...
            lwm2m_sdk_notify_regup_failed(packet?packet->code:COAP_IGNORE,"Registration update failed");
        }
    }
}
                                  ...
                                  bool withObjects)
{
	...
	if(strlen(g_szEPI)>0)
	{
		...
		plat_sprintf(query,"%sepi=%s",QUERY_STARTER,g_szEPI);
		LOG_ARG("-------query=%s-------\n",query);
		coap_set_header_uri_query(transaction->message, query);
		//coap_set_header_content_type(transaction->message, LWM2M_CONTENT_LINK);
	}
